...
def make_request(baseurl, params):
    ...
    return requests.get(baseurl,params=params,auth=oauth).json()
...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    if uniq_key in CACHE_DICT.keys():
        ...
        CACHE_DICT[uniq_key] = make_request(baseurl,params)
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    for tweet in tweets:
        ...
        for hashtag in hashtags:
            if hashtag['text'].lower() != hashtag_to_ignore:
                hashtags_list.append(hashtag['text'])
    most_common = Counter(hashtags_list).most_common(1)
